...
export class Screen {
    ...
    constructor(size: Size, font: Font) {
        this.font = font;
        this.widthInCharacters = size.w;
        this.heightInCharacters = size.h;
    ...
    async init(containerEl: HTMLElement) {
        ...
        this.setScreenMode(this.getSizeInCharacters(), this.font);
    }
    ...
    update(buffer: TextBuffer) {
        ...
        if (buffer.bellRequested) {
            const beepEl = document.getElementById(
                "bell",
        ...
        for (
            let y = 0;
            y < this.heightInCharacters && y < page.size.h;
            y += 1
        ) {
            for (
                let x = 0;
                x < this.widthInCharacters && x < page.size.w;
                x += 1
            ) {
                ...
                const newCharacter: ScreenBufferCharacter = {
                    attributes:
                        getScreenCharacterAttributesFromTermCellAttributes(
                            cellAttr,
                        ),
                    character: cell.rune,
                };
