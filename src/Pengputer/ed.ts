...
export class EdApp implements Executable {
    ...
    private writeFile(fileName: string | null): number {
        ...
        if (!wfile) {
            try {
                ...
                if (file) {
                    ...
                    std.writeConsole(
                        wpath +
                            ": i TRIED to open the FILE but it REALLY didn't work\n",
                    );
        ...
        if (this.file) {
            if (!this.path)
                throw new Error("EDeezNuts violation: illegal right nut state");
        } else {
    ...
    async run(args: string[]) {
        ...
        const isalnum = (c: string) =>
            48 <= c.charCodeAt(0) && c.charCodeAt(0) <= 57;
        ...
        while (true) {
            ...
            if (cmd == "q") {
                ...
                if (lines.length == 0) {
                    ...
                    this.lines = [
                        ...this.lines.slice(0, this.lineNo),
                        ...latter,
                    ];
                }
                continue;
                ...
                this.lines = [
                    ...this.lines.slice(0, this.lineNo - 1 + i),
                    ...latter,
                ];
