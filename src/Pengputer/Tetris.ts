...
class FallingPiece {
    ...
    private updateGhost() {
        ...
        while (!this.ghost.getIsColliding()) {
            this.ghost.position = vectorAdd(this.ghost.position, {
                x: 0,
                y: 1,
            });
            hasMovedDown = true;
    ...
    public update(dt: number) {
        ...
        if (this.msPerCell === 0) {
            ...
            while (this.stepCounter <= 0) {
                this.stepCounter += this.isPushdown
                    ? PUSHDOWN_LENGTH
                    : this.msPerCell;
                this.step();
            }
    ...
    public clearLines() {
        ...
        while (y > 0) {
            ...
            for (let x = 0; x < WIDTH; x += 1) {
                if (
                    !this.board[this.getBoardIndexFromBoardPosition({ x, y })]
                        .filled
                ) {
                    isRowComplete = false;
                }
            ...
            if (isRowComplete) {
                ...
                for (let y2 = y; y2 > 0; y2 -= 1) {
                    for (let x = 0; x < WIDTH; x += 1) {
                        this.board[
                            this.getBoardIndexFromBoardPosition({ x, y: y2 })
                        ] = cellClone(
                            this.board[
                                this.getBoardIndexFromBoardPosition({
                                    x,
                                    y: y2 - 1,
                                })
                            ],
                ...
                for (let x = 0; x < WIDTH; x += 1) {
                    this.board[
                        this.getBoardIndexFromBoardPosition({ x, y: 0 })
                    ] = this.getEmptyCell();
                }
            } else {
                y -= 1;
...
class Tetris implements GameState {
    ...
    constructor(pc: PC) {
        ...
        this.boardScreenRect = {
            x: Math.round(
                (screenSize.w - WIDTH * CELL_WIDTH - BORDER_SIZE_H) / 2,
            ),
    ...
    public drawBoardSquare(boardPosition: Vector, color: PieceColor) {
        const screenPos =
            this.getScreenPositionFromBoardPosition(boardPosition);
        if (screenPos) {
                ...
                this.drawSquare(
                    vectorAdd(
                        vectorMultiplyComponents(
                            vectorAdd(
                                localSquarePosition,
                                piece.getDescriptor().nextOffset,
                            ),
                            {
                                x: CELL_WIDTH,
                                y: CELL_HEIGHT,
...
class MainMenu implements GameState {
    ...
    onEnter() {
        ...
        std.setConsoleCursorPosition({
            x: Math.floor(
                (std.getConsoleSize().w -
                    "Original game: Alexey Pajitnov".length) /
                    2,
            ),
            y: start + 13,
        });
