...
class Pengsweeper extends State {
    ...
    override onEnter() {
        ...
        const clickListener: ClickListener = ({ position }) => {
            const boardPosition = vectorSubtract(
                position,
                this.getFieldOrigin(),
            );
            if (
        ...
        this.unsubscribeFromClicks =
            std.addMouseScreenClickListener(clickListener);
    }
    ...
    private explodeCell(pos: Vector) {
        ...
        if (!explodedCell || !explodedCell.isMine) {
            throw new Error(
                "Attempting to explode a mine that does not exist.",
            );
        }
    ...
    private drawStatus() {
        ...
        std.writeConsole(
            _.padStart(String(this.minesCount - this.flagCount), 3),
            {
